...
# ./rqlited -node-id 1 -http-addr 127.0.0.1:8001 -raft-addr 127.0.0.1:8002 C:\Users\Timo\AppData\Local\Programs\rqlite\db

# Optional: Specify the path to the database file (C:\Users\Timo\AppData\Local\Programs\rqlite\db), the node id (-node-id 1) and the addresses (-http-addr and -raft-addr).
...
# b) connect to node with `rqlite -H 127.0.0.1 -p 8001`
# c) execute SQL commands

# 2. Run the following code to create a new planet and get all planets:

url_execute = 'http://127.0.0.1:8001/db/execute'
url_query = 'http://127.0.0.1:8001/db/query'
...
params_post = json.dumps([
    """INSERT INTO planets (from_sun, name) VALUES (4, "Mars");"""
])
